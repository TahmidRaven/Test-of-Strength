...
goblins = []  # List to store multiple goblins
goblin_speed = 2  
...
def spawn_goblins():
    goblins.clear()
    # Add a few goblins at different positions
    goblins.append({"x": 200, "y": 400})
    goblins.append({"x": 500, "y": 400})
    goblins.append({"x": 350, "y": 400})
...
def update(value):
    if not pause and not game_over and not game_won:
        witch_shoot()
        if knight_alive:
            knight_shoot()
            update_knight()
        ...
        for goblin in goblins:
            dx = player_x - goblin["x"]
            dy = player_y - goblin["y"]
            length = math.sqrt(dx*dx + dy*dy)
            if length > 0:
                goblin["x"] += (dx/length) * goblin_speed
                goblin["y"] += (dy/length) * goblin_speed
                
        # Check goblin collision with player
        for goblin in goblins[:]:  # Use slice copy to safely modify list while iterating
            distance = math.sqrt((goblin["x"] - player_x)**2 + (goblin["y"] - player_y)**2)
            if distance < 20:  # If goblin touches player
                game_over = True        
    glutPostRedisplay()
